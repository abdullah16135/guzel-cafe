...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const path = `offers/${id}/${randomUUID()}.${ext}`;
  ...
  const { error: offerError } = await supabaseAdmin.from("offers").update({ media_id: media.id }).eq("id", id);
  if (offerError) return NextResponse.json({ error: offerError.message }, { status: 400 });
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await supabaseAdmin.from("offers").update({ media_id: null }).eq("id", id);

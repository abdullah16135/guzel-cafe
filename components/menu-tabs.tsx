...
export function MenuTabs({
  ...
  const shouldScrollToSectionRef = useRef<string | null>(null);
  ...
  function sectionOffersFor(sectionId: string) {
    return offers.filter((offer) => (offer.section_ids ?? []).includes(sectionId));
  }
  ...
  function warmGroup(group: MenuMainCategoryGroup | null | undefined) {
    ...
    const firstSections = group.subcategories.slice(0, 3);
    ...
    preloadImages([
      ...
      ...group.subcategories.slice(0, 6).map((section) => section.media?.public_url),
      ...currentProducts.map((product) => product.media?.public_url),
      ...currentSectionOffers.slice(0, 6).map((offer) => offer.media?.public_url),
    ]);
  }

  function warmSection(
    section:
      | (typeof activeGroup extends null ? never : NonNullable<typeof activeGroup>["subcategories"][number])
      | null
      | undefined
  ) {
    if (!section) return;
    ...
    preloadImages([
      ...
      ...section.products.slice(0, 10).map((product) => product.media?.public_url),
  ...
  function selectCategory(nextCategory: string) {
    ...
    shouldScrollToSectionRef.current = null;
    setActiveCategoryKey(nextCategory);

    const nextGroup = groups.find((group) => groupKey(group) === nextCategory) ?? null;
    const firstSection = nextGroup?.subcategories[0] ?? null;
    const firstSectionKey = firstSection ? sectionKey(firstSection) : null;

    setActiveSectionKey(firstSectionKey);
    updateQuery(nextCategory, firstSectionKey);
    warmGroup(nextGroup);
  }
  ...
  function selectSection(nextSectionKeyValue: string) {
    ...
    const targetSection =
      activeGroup.subcategories.find((section) => sectionKey(section) === nextSectionKeyValue) ?? null;

    if (!targetSection) return;

    setActiveSectionKey(nextSectionKeyValue);
    shouldScrollToSectionRef.current = targetSection.id;
    updateQuery(groupKey(activeGroup), nextSectionKeyValue);
    warmSection(targetSection);
  ...
  useEffect(() => {
    if (!activeGroup || !shouldScrollToSectionRef.current) return;

    const frame = window.requestAnimationFrame(() => {
      scrollToSection(shouldScrollToSectionRef.current!);
      shouldScrollToSectionRef.current = null;
    });
  ...
  useEffect(() => {
    ...
    preloadImages([
      ...
      ...activeGroup.subcategories.slice(0, 6).map((section) => section.media?.public_url),
      ...groupLevelOffers.slice(0, 4).map((offer) => offer.media?.public_url),
    ]);

    activeGroup.subcategories.slice(0, 3).forEach((section) => warmSection(section));
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
                      ...
                      {group.description ? (
                        <p
                          className={cn(
                            ...
                            active ? "text-white/74" : "text-maroon/58"
                          )}
                        >
                          {group.description}
                        </p>
      ...
      {activeGroup ? (
        <section
          className="overflow-hidden rounded-[26px] border border-maroon/10 bg-white shadow-[0_12px_30px_rgba(115,18,38,0.06)]"
          ref={detailsRef}
        >
          ...
          <div className="border-t border-maroon/8 px-3 py-4 sm:px-4 sm:py-5">
            <div className="space-y-6 sm:space-y-7">
              {activeGroup.subcategories.map((section, sectionIndex) => {
                const sectionOffers = sectionOffersFor(section.id);
                const isSectionActive = sectionKey(section) === activeSectionKey;
                ...
                return (
                  <section
                    ...
                    className={cn(
                      "scroll-mt-32 rounded-[24px] border p-3 sm:p-4 transition-colors",
                      isSectionActive ? "border-maroon/15 bg-[#fffaf8]" : "border-maroon/8 bg-white"
                    )}
                  >
                    <div className="mb-3 flex items-end justify-between gap-3">
                      <div>
                        ...
                        <h4 className="mt-1 font-serif text-[1.35rem] text-maroon">{section.label}</h4>
                        {section.description ? (
                          <p className="mt-1 text-sm leading-6 text-maroon/58">{section.description}</p>
                        ) : null}
                      </div>
                      <span className="text-sm text-maroon/52">{section.products.length}</span>
                    </div>
                    ...
                    <div className="grid gap-2.5 sm:gap-3">
                      {section.products.map((product, index) => (
                        <ProductCard
                          ...
                          locale={locale}
                          compact
                          priority={sectionIndex === 0 ? index < 3 : index < 1}
                        />
                      ))}
                    </div>
                  </section>

...
const warmedProductImages = new Set<string>();

function warmProductImage(src?: string | null) {
  const resolved = resolveMediaUrl(src);
  if (!resolved || warmedProductImages.has(resolved)) return;

  warmedProductImages.add(resolved);
  const img = new Image();
  img.src = resolved;
}
...
export function ProductCard({
  ...
  const sizes = useMemo(
    () =>
      [...(product.product_sizes ?? [])].sort(
        (a, b) => Number(b.is_default) - Number(a.is_default) || a.sort_order - b.sort_order
      ),
  ...
  const imageSrc = product.media?.public_url;

  return (
    <Link
      href={`/menu/${product.slug}`}
      className="block overflow-hidden rounded-[22px] border border-maroon/10 bg-white shadow-[0_8px_22px_rgba(115,18,38,0.05)] active:scale-[0.995]"
      onMouseEnter={() => warmProductImage(imageSrc)}
      onTouchStart={() => warmProductImage(imageSrc)}
      onFocus={() => warmProductImage(imageSrc)}
    >
      <article
        className={`grid gap-0 ${
          compact ? "grid-cols-[96px_minmax(0,1fr)]" : "grid-cols-[112px_minmax(0,1fr)] sm:grid-cols-[128px_minmax(0,1fr)]"
        }`}
      >
        ...
        <div className="flex min-w-0 flex-col justify-between p-3.5">
          <div className="space-y-1.5">
            <div className="flex items-start justify-between gap-3">
              <h3 className="min-w-0 flex-1 font-serif text-[1rem] leading-6 text-maroon sm:text-[1.08rem]">
                {name}
              </h3>
              {activePrice ? (
                <span className="shrink-0 text-sm font-semibold text-maroon">{formatCurrency(activePrice)}</span>
              ) : null}
            </div>
            ...
            {description ? (
              <p className="line-clamp-2 text-[13px] leading-5 text-maroon/66">{description}</p>
            ) : null}
          </div>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          isActive
                            ...
                            : "border-maroon/12 bg-[#fff8f5] text-maroon/72"
                        )}

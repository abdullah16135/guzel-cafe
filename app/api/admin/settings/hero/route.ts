...
export async function POST(request: Request) {
  ...
  const path = `branding/hero-${randomUUID()}.${ext}`;
  ...
  const { data: media, error: mediaError } = await supabaseAdmin.from("media").insert({ bucket: "guzil-media", path, public_url: pub.publicUrl, alt_ar: "صورة الهيرو", alt_en: "Hero image" }).select().single();
  if (mediaError) return NextResponse.json({ error: mediaError.message }, { status: 400 });
  const { error: settingsError } = await supabaseAdmin.from("settings").update({ hero_media_id: media.id }).eq("id", SETTINGS_ID);

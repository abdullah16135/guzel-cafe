    ...
    >
      ...
      <div className="relative flex h-full w-full flex-col items-center justify-center gap-3 px-5 py-6 text-center">
        ...
        <div className="space-y-1.5">
          <p className={cn("line-clamp-2 font-serif text-lg leading-snug", fallbackTone === "warm" ? "text-maroon/88" : "text-white")}>{label || alt}</p>
          <p className={cn("text-[11px] font-semibold uppercase tracking-[0.24em]", fallbackTone === "warm" ? "text-maroon/45" : "text-white/65")}>Güzel Café</p>
        </div>
      </div>
      ...
      {hasImage ? (
        <>
          ...
          />
          <div className={cn("absolute inset-0 transition duration-300", isLoaded ? "opacity-0" : "opacity-100") }>
